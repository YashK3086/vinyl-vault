    ...
    <div className="w-full relative rounded-2xl overflow-hidden glass-panel border-zinc-700 p-[1px]">
      ...
      <div className="w-full bg-[#1a1412] p-6 sm:p-8 flex flex-col gap-8 relative overflow-hidden rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center gap-1 z-10 select-none">
          <span className="text-xs font-black text-amber-600/80 font-mono tracking-[0.25em] uppercase">
            ✦ THE VINYL VAULT TOUR PRESENTS ✦
          </span>
          <h2 className="text-4xl sm:text-5xl font-black tracking-tighter text-zinc-300 uppercase font-display leading-none mt-1.5">
            YASHVARDHAN KHANNA
          </h2>
          <div className="flex items-center gap-3 mt-2">
            <span className="h-[1px] w-12 bg-zinc-850" />
            <span className="text-xs text-zinc-500 font-bold font-mono tracking-widest uppercase">
          ...
          <div className="flex flex-col items-center gap-3 z-10 w-full">
            ...
            <div className="flex flex-col gap-6 w-full max-w-[550px] mt-3">
              {education.map((edu, idx) => (
                ...
                >
                  <h4 className="text-base font-extrabold text-zinc-300 uppercase tracking-wide">
                    {edu.institution}
                  </h4>
                  <p className="text-sm text-amber-600/80 font-bold font-mono">
                    {edu.degree}
                  </p>
                  <div className="flex items-center gap-2 text-xs text-zinc-500 font-semibold font-mono mt-1">
          ...
          <div className="flex flex-col items-center gap-4 z-10 w-full border-t border-dashed border-zinc-950 pt-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3.5 max-w-[600px] mt-2">
              {certifications.map((cert, idx) => (
                <div 
                  key={idx} 
                  className="flex items-center gap-2 text-sm text-zinc-350 font-mono font-semibold hover:text-zinc-300 hover:scale-102 transition-all"
                >
                  <ShieldCheck className="w-4.5 h-4.5 text-amber-600/80 flex-shrink-0" />
                  <span>{cert.name}</span>
                  {cert.credlyUrl && (
                    <a 
                      href={cert.credlyUrl} 
                      ...
                      className="text-zinc-550 hover:text-amber-600/80 transition-colors"
                  ...
                  {cert.status === "Pursuing" && (
                    <span className="text-[9.5px] bg-zinc-950 text-zinc-500 font-bold font-mono px-1.5 py-0.5 rounded uppercase tracking-wide">
                      {cert.status}
                    </span>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full max-w-[700px] mt-2 text-center">
              {leadershipAchievements.map((lead, idx) => (
                ...
                >
                  ...
                  <p className="text-xs text-zinc-500 font-semibold font-mono mb-1 uppercase">
                    {lead.organization}
                  </p>
        ...
        <div className="w-full flex flex-col items-center gap-4 z-10 border-t border-zinc-950 pt-6 mt-2 relative select-none">
          ...
          <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-[10.5px] text-zinc-500 px-4">
            ...
            <div className="flex items-center gap-0.5 h-6 bg-zinc-950 px-2 py-1 rounded border border-zinc-800/50">
              ...
              <div className="w-[1px] h-full bg-zinc-650" />
              <div className="w-[3px] h-full bg-zinc-650" />
              <span className="text-[9px] text-zinc-500 ml-2 font-black tracking-wider">ADMIT ONE - PORTFOLIO GUEST</span>
            </div>
