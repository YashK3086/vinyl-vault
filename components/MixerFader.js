...
export default function MixerFader({ activeSection, setActiveSection }) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-6 p-5 rounded-2xl bg-zinc-950 border border-zinc-800 shadow-xl w-full">
      
      {/* Visual Mixer Strip Branding */}
      <div className="flex flex-row md:flex-col justify-between md:justify-center items-center gap-3 w-full md:w-auto md:border-r md:border-zinc-900 md:pr-6 md:min-h-[180px]">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-amber-500 shadow-inner flex-shrink-0">
            <Volume2 className="w-4 h-4 animate-pulse" />
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-black font-mono tracking-widest text-white uppercase leading-none">
              CHANNEL SELECT
            </span>
            <span className="text-[10px] font-mono text-zinc-300 mt-1 uppercase tracking-wider font-bold">
              INPUT FADER
            </span>
          </div>
        </div>
        ...
        {/* Status display */}
        <div className="bg-zinc-900/60 border border-zinc-850 px-3 py-1.5 rounded font-mono text-xs text-zinc-200 flex items-center gap-1.5 md:mt-4">
      ...
      <div className="flex-1 grid grid-cols-12 items-center gap-6 w-full select-none">
        ...
        <div className="col-span-4 flex justify-center py-2">
          <div 
            ref={containerRef}
            className="relative w-8 h-[184px] bg-zinc-900 rounded-lg border border-zinc-800 flex justify-center py-4 shadow-inner"
          >
            {/* Center metal slot track */}
            <div className="absolute top-4 bottom-4 w-1 bg-zinc-950 border-x border-zinc-850 rounded" />
            
            {/* Level tick marks */}
            <div className="absolute inset-y-4 left-1 flex flex-col justify-between font-mono text-[9.5px] text-zinc-400 select-none pointer-events-none font-bold">
              <span>+6</span>
              <span>+3</span>
              <span>0</span>
              <span>-3</span>
            ...
            <div className="absolute inset-y-4 right-1 flex flex-col justify-between font-mono text-[9.5px] text-zinc-400 select-none pointer-events-none font-bold">
              <span>dB</span>
              <span>—</span>
              <span>—</span>
              <span>—</span>
            ...
            <motion.div
              ...
              style={{ y }}
              className="absolute left-1/2 -translate-x-1/2 w-6 h-8 bg-gradient-to-b from-zinc-700 via-zinc-850 to-zinc-900 rounded border border-zinc-600/60 shadow-lg cursor-grab active:cursor-grabbing flex items-center justify-center z-10 hover:border-amber-500/40"
            >
              {/* Metallic center line */}
              <div className="w-full h-[2px] bg-amber-500 shadow-[0_0_6px_#f59e0b]" />
              ...
              >
                <div className="flex flex-col gap-0.5">
                  <span className={`text-xs font-extrabold font-mono tracking-wider transition-colors ${
                    isActive ? "text-amber-400 glow-text-amber" : "text-zinc-300 group-hover:text-white"
                  }`}>
                    {titles[sec]}
                  </span>
                  <span className="text-xs text-zinc-200 font-sans font-semibold leading-relaxed">
                    {subtitles[sec]}
                  </span>
                </div>
