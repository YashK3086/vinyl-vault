...
async function run() {
  ...
  for (let i = 0; i < tracksConfig.length; i++) {
    const t = tracksConfig[i];
    await sleep(250);

    const results = await fetchItunes(t.q);
    const targetClean = clean(t.title.replace(/\(.*?\)/g, '').replace(/\|.*/, ''));
    const artistClean = clean(t.artist);

    // Filter results that have audio preview
    const audioResults = results.filter(r => r.previewUrl);

    // 1st preference: exact track name match
    let match = audioResults.find(r => clean(r.trackName) === targetClean);
    
    // 2nd preference: includes track name and artist name
    if (!match) {
      match = audioResults.find(r => clean(r.trackName).includes(targetClean) && clean(r.artistName).includes(artistClean));
    }
    
    // 3rd preference: includes track name
    if (!match) {
      match = audioResults.find(r => clean(r.trackName).includes(targetClean) || targetClean.includes(clean(r.trackName)));
    }

    // 4th preference: first result with preview
    if (!match && audioResults.length > 0) {
      match = audioResults[0];
    }

    // Fallback if none had previewUrl
    if (!match && results.length > 0) {
      match = results[0];
    }

    const uniqueId = `track_${clean(t.artist)}_${clean(t.title)}`;
    ...
    console.log(`[${i + 1}/70] [${t.artist}] "${t.title}" -> "${match?.trackName}" | Audio: ${!!previewUrl}`);
  }
