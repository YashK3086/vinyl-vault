...
const tracksConfig = [
  ...
  { artist: "Mac Miller", title: "2009", q: "Mac Miller Swimming 2009" },
  { artist: "Mac Miller", title: "Good News", q: "Mac Miller Good News Circles" },
...
function scoreMatch(targetArtist, targetTitle, r) {
  ...
  return (titleScore * 3.0) + (artistScore * 1.5) + bonus;
}
...
async function run() {
  ...
  for (let i = 0; i < tracksConfig.length; i++) {
    ...
    await sleep(350); // Respectful rate limit delay

    let results = await fetchItunes(t.q);
    if (results.length === 0) {
      await sleep(500);
      results = await fetchItunes(`${t.artist} ${t.title}`);
    }
    ...
    const uniqueId = `trk_${t.artist.toLowerCase().replace(/[^a-z0-9]/g, '_')}__${t.title.toLowerCase().replace(/[^a-z0-9]/g, '_')}`;
    ...
    if (previewUrl && previewMap.has(previewUrl)) {
      console.warn(`DUPLICATE: [${t.artist}] "${t.title}" shares with "${previewMap.get(previewUrl)}"`);
    } else if (previewUrl) {
    ...
    console.log(`[${i + 1}/70] [${t.artist}] "${t.title}" -> "${best?.trackName}" | Audio: ${!!previewUrl}`);
  }
...
`;
  ...
  console.log(`\nALL 70 TRACKS GENERATED! Wrote data/musicLibrary.js.`);
}
